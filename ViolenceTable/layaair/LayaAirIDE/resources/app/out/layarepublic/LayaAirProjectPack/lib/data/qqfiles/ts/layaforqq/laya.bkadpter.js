...
(function(window,document,Laya){
	...
	var Bitmap=laya.resource.Bitmap,Browser=laya.utils.Browser,Canvas=BK.Canvas,Color$1=laya.utils.Color;
	var Event=laya.events.Event,EventDispatcher=laya.events.EventDispatcher,Font=laya.display.css.Font,HTMLCanvas=laya.resource.HTMLCanvas;
	var Handler=laya.utils.Handler,HitArea=laya.utils.HitArea,Input=laya.display.Input,Loader=laya.net.Loader;
	var LocalStorage=laya.net.LocalStorage,Matrix=laya.maths.Matrix,MouseManager=laya.events.MouseManager,Point=laya.maths.Point;
	var Rectangle=laya.maths.Rectangle,Render=laya.renders.Render,RenderContext=laya.renders.RenderContext,RunDriver=laya.utils.RunDriver;
	var Socket=laya.net.Socket,SoundChannel=laya.media.SoundChannel,SoundManager=laya.media.SoundManager,Sprite=laya.display.Sprite;
	var Stage=laya.display.Stage,Stat=laya.utils.Stat,Text=laya.display.Text,Texture=laya.resource.Texture,TouchManager=laya.events.TouchManager;
	var URL=laya.net.URL,Utils=laya.utils.Utils,WebGL=laya.webgl.WebGL;
/**
*...
*@author ww
*/
...
var BKContext=(function(){
	...
	__proto.measureText=function(text){
		...
		console.log("measureText:",text,BKContext._textStyle.fontSize,rst.contentWidth,rst.contentHeight,rst);
		return rst;
...
var BKHTMLImage=(function(){
	...
	__getset(0,__proto,'src',function(){
		...
		if (this._src.indexOf("http")>=0){
			BKFileCache.getHttpFile(this._src,Handler.create(this,this.__onHttpLoaded));
		}else
...
var BKWebSocket=(function(){
	function BKWebSocket(url){
		...
		this._socket.onMessage=function (ws,data){
			if (data.isBinary){
				data.data=BKLaya.tools.BinaryLoader.BufferToArrayBuffer(data.data);
			}
	...
	__proto.send=function(data){
		...
		if((data instanceof ArrayBuffer)){
			this._socket.send(BKLaya.tools.BinaryLoader.arrayBufferToBKBuffer(data));
		}
	}
...
var BKFileCache=(function(){
	...
	BKFileCache.getHttpFile=function(url,completeHandler){
		...
		function __ondataLoaded (res,code){
			BK.FileUtil.writeBufferToFile("GameSandBox://test.png",res);
			completeHandler.runWith("GameSandBox://test.png");
		}
	}
...
var LoaderAdpt=(function(){
	...
	*/
	...
	LoaderAdpt.__init=function(){
		...
		if (!Loader)return;
		Loader["prototype"]["_loadSound"]=LoaderAdpt["prototype"]["_loadSound"];
	}
...
var LocalStorageAdpt=(function(){
	...
	LocalStorageAdpt._readData=function(){
		...
		try{
			return JSON.parse(BK.FileUtil.readFile(LocalStorageAdpt.dataPath).readAsString());
			}catch (e){
			return null;
		}
	}
...
var RenderContextAdpt=(function(){
	...
	//FunctionReplaceTool.replaceClassFuns(RenderContext,RenderContextAdpt,replaceFuns);
...
var SocketAdpt=(function(){
	...
	SocketAdpt.__init=function(){
		if (SocketAdpt._preConnect)return;
		if (!Socket)return;
...
var SpriteAdpt=(function(){
	...
	*/
	...
	SpriteAdpt.__init=function(){
		FunctionReplaceTool.replaceFunction(Sprite,SpriteAdpt,"drawToCanvas");
	}
...
var StageAdpt=(function(){
	...
	*/
	__proto.setScreenSize=function(screenWidth,screenHeight){
		scaleMode=/*laya.display.Stage.SCALE_FULL*/"full";
		var rotation=false;
		if (this._screenMode!==/*laya.display.Stage.SCREEN_NONE*/"none"){
...
var FunctionReplaceTool=(function(){
	...
	FunctionReplaceTool.replaceGetSet=function(clzPre,getFun,setFun){}
...
var LayaBKAdpter=(function(){
	...
	LayaBKAdpter._adptLayaInit=function(width,height){
		WebGL.enable();
		LayaBKAdpter._preLayaInit(LayaBKAdpter.getScreenWidth(),LayaBKAdpter.getScreenHeight());
...
var BKCanvasRenderContext=(function(_super){
	...
	__proto.sizefun=function(w,h){
		console.log("Canvas.sizeFun:",w,h);
	...
	__getset(0,__proto,'font',null,function(value){
		...
		console.log("set text size:",style.fontSize);
...
var BKSoundChannel=(function(_super){
	...
	*/
	__proto.play=function(){
		...
		this._audio=new BK.Audio(type,this.url,this.loops==0?-1:this.loops);
