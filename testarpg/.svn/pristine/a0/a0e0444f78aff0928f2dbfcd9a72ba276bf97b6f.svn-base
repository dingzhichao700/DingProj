...
module egret {
	 ...
	 * @see #close()
	 */
	export function openWindow(targetClass:any,isUI:boolean = true,layerType:number = -1,align:number = -1,name:string = null,isNew:boolean = false):DisplayObject{
		return ApplicationManager.getInstance().open(targetClass,layerType,align,name,isNew);
	}
}