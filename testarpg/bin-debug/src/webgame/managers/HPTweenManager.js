...
(function (egret) {
    ...
    var HPTweenManager = (function () {
        ...
        __egretProto__.tweenExecute = function (container, x, y, radian, radius, text, color, size, cacheId) {
            ...
            var damage = this.getDamageItem(x, y, text, color, size);
            container.addChild(damage);
            var tx = Math.cos(radian) * radius + (x - damage.width / 2);
            ...
            toParam.y = ty;
            params[3] = toParam;
            egret.Tween.get(damage).to(toParam, 1000, egret.Ease.circInOut).call(this.randomDirComplete, this, params);
        };
        /**
         * 飘血动画结束，回收文本对象
         ...
         */
        __egretProto__.randomDirComplete = function (damage, cacheId, cacheId2, toParam) {
            if (damage.parent)
        ...
        __egretProto__.tweenLine = function (container, x, y, radius, text, color, size) {
            ...
            toParam.y = ty;
            params[3] = toParam;
            egret.Tween.get(damage).to(toParam, 1000).call(this.randomDirComplete, this, params);
        };
        /**
         * 获取文本对象
         * @param x 文本x
         ...
         * @returns {TextField}
         */
        __egretProto__.getDamageItem = function (x, y, text, color, size) {
            if (color === void 0) { color = 0xffff00; }
            if (size === void 0) { size = 20; }
            var damage = this._damageFields.pop();
            if (!damage)
                damage = new egret.DamageItem();
