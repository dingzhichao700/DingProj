package laya.vv.mini
{
	...
	public class MiniFileMgr{
		/**@private 读取文件操作接口**/
		private static var fs:* = VVMiniAdapter.window.qg;
		/**@private 下载文件接口**/
		...
		public static function read(filePath:String, encoding:String = "ascill", callBack:Handler = null, readyUrl:String = "",isSaveFile:Boolean = false,fileType:String = ""):void {
			...
			fs.readFile({uri: fileUrl, encoding: encoding, success: function(data:Object):void {
				if(!data.data)
					data.data = data.text;
		...
		public static function readFile(filePath:String, encoding:String = "ascill", callBack:Handler = null, readyUrl:String = "",isSaveFile:Boolean = false,fileType:String = "",isAutoClear:Boolean =true):void {
			fs.readFile({uri: filePath, encoding: encoding, success: function(data:Object):void {
			...
			}else
			{
				fs.getFileInfo({
					...
					success:function(data:Object):void
					{
						...
						fs.copyFile({srcUri: tempFilePath, dstUri: saveFilePath, success: function(data2:Object):void {
		...
		public static function deleteFile(tempFileName:String, readyUrl:String = "", callBack:Handler = null,encoding:String = "",fileSize:int = 0):void {
			...
			fs.deleteFile({uri: deleteFileUrl, success: function(data:Object):void {
				var isAdd:Boolean = tempFileName != "" ? true : false;
				if(tempFileName != "")
				{
					var saveFilePath:String = getFileNativePath(tempFileName);
					fs.copyFile({srcUri: tempFileName, dstUri: saveFilePath, success: function(data:Object):void {
		...
		private static function writeFilesList(fileurlkey:String,filesListStr:String,isAdd:Boolean):void
		{
			...
			fs.writeFile({uri: listFilesPath, encoding: 'utf8', data: filesListStr, success: function(data:Object):void {
		...
		public static function existDir(dirPath:String, callBack:Handler):void {
			fs.mkdir({uri: dirPath, success: function(data:Object):void {
		...
		public static function readSync(filePath:String, encoding:String = "ascill", callBack:Handler = null, readyUrl:String = ""):void {
			...
			try
			{
				filesListStr = fs.readFileSync({uri:fileUrl,encoding:encoding});
